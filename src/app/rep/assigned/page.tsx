...
interface AssignedStudent {
  ...
  matches?: {
    primary?: { program?: string; reason?: string };
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {students.map((s) => {
          const decision = s.representativeReview?.decision || s.representative?.status;
          const comments = s.representativeReview?.comments || s.representative?.comments;
          ...
          const english = s.englishTest || s.englishProficiency || 'English';
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    {decision && (
                      <Badge
                        variant={
                          decision.toLowerCase() === 'green'
                            ? 'green'
                            : decision.toLowerCase() === 'yellow'
                            ? 'yellow'
                            : 'red'
                        }
                      >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    <div className="flex items-center gap-1.5 text-dim-grey">
                      ...
                      GPA: <span className="font-medium text-carbon">{s.gpa ?? 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-dim-grey">
                    ...
                    <div className="flex items-center gap-1.5 text-dim-grey">
                      📝 {english}: <span className="font-medium text-carbon">{s.englishScore ?? 'N/A'}</span>
                    </div>
                  </div>

                  {s.assessment?.totalScore != null && (
                    <div className="mt-2">
                  ...
                  {comments && (
                    <div className="mt-2 bg-porcelain rounded-lg px-3 py-2">
                      ...
                      <p className="text-sm text-carbon">{comments}</p>
                    </div>
                  )}
                </div>
              </div>
